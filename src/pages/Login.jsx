...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    else if (!emailRegex.test(email)) newErrors.email = "Email không hợp lệ! Chỉ hỗ trợ Gmail.";
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (data.success) { setMessage("✅ Mã OTP đã gửi đến email của bạn!"); setStep(2); }
  ...
  const handleVerifyOtp = async () => {
    ...
    if (!cleanOtp) return setMessage("⚠️ Vui lòng nhập mã OTP!");
    setOtp(cleanOtp);
    setLoading(true);
    try {
      ...
      const data = await res.json();
      if (data.success) { setToken(data.token); setStep(3); setMessage("✅ OTP chính xác! Hãy nhập mật khẩu mới."); }
      else setMessage(data.message);
    } catch {
  ...
  const handleResetPassword = async () => {
    if (!newPassword || !confirmPassword) return setMessage("⚠️ Vui lòng nhập đầy đủ mật khẩu mới!");
  ...
  const checkStrength = (password) => {
    ...
    if (passed === 4) return { strength: "Mạnh 💪", color: "bg-green-500" };
    if (passed === 3) return { strength: "Trung bình ⚡", color: "bg-yellow-400" };
    if (passed === 2) return { strength: "Yếu ⚠️", color: "bg-orange-400" };
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <input
          ...
          className={`w-full bg-white/20 border ${errors.email ? "border-red-400" : "border-gray-300/30"} rounded-lg p-3 text-white placeholder-gray-300 focus:ring-2 focus:ring-yellow-400 outline-none`}
        />
        {errors.email && <p className="text-red-400 text-sm mt-1">{errors.email}</p>}
      </div>

      <div>
        <label className="block text-sm font-semibold mb-1 text-gray-200">Mật khẩu</label>
        <div className="relative">
          <input
            ...
            className={`w-full bg-white/20 border ${errors.password ? "border-red-400" : "border-gray-300/30"} rounded-lg p-3 pr-10 text-white placeholder-gray-300 focus:ring-2 focus:ring-yellow-400 outline-none`}
          />
          <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-gray-300 hover:text-yellow-300">
      ...
      {errors.general && <p className="text-red-400 text-center text-sm mt-2">{errors.general}</p>}

      <button type="submit" disabled={loading} className="w-full py-3 bg-gradient-to-r from-yellow-400 to-orange-500 text-gray-900 font-bold rounded-lg shadow-lg hover:scale-[1.03] transition-transform">
        {loading ? "Đang đăng nhập..." : "Đăng nhập"}
      </button>

      <button type="button" onClick={handleSendOtp} className="w-full text-yellow-300 hover:underline mt-3">
        Quên mật khẩu?
      </button>
    </form>
  );
  ...
  const renderOtpForm = () => (
    <div className="space-y-4">
      ...
      <div className="flex justify-between">
        <button onClick={handleVerifyOtp} className="px-4 py-2 bg-yellow-400 text-gray-900 font-bold rounded-lg hover:scale-[1.03] transition">
          Xác minh OTP
        </button>
        <button onClick={() => setStep(1)} className="text-gray-200 text-sm hover:underline">Hủy</button>
      </div>
    </div>
  );
  ...
  const renderResetForm = () => (
    <div className="space-y-4">
      <label className="block text-sm font-semibold mb-1 text-gray-200">Mật khẩu mới</label>
      <div className="relative">
        <input
      ...
      <label className="block text-sm font-semibold mb-1 text-gray-200 mt-4">Xác nhận mật khẩu mới</label>
      <div className="relative">
        <input
